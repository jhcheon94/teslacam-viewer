...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(await page.locator('#viewsw button').evaluateAll(buttons => buttons.map(button => button.dataset.v)), ['timeline', 'list', 'map']);
    ...
    assert.match(await page.locator('#journey-hover').innerText(), /^2026-09-01 08:\d{2}:\d{2}$/);
    await page.mouse.move(journeyBox.x, journeyBox.y - 40);
    assert.equal(await page.locator('#journey-hover').evaluate(el => el.classList.contains('show')), false);
