...
| 저장 이벤트 | `SavedClips` | 운전자가 대시캠 아이콘·화면·경적 등의 설정으로 보존한 영상입니다. RecentClips에서 저장된 영상이 이곳으로 이동합니다. | 위치 기록이 있으면 읽기 쉬운 **주소**로 변환해 표시합니다. 기록이 없으면 날짜·시각만 표시합니다. |
| 센트리 이벤트 | `SentryClips` | 주차 중 센트리 모드가 감지한 보안 이벤트 영상입니다. | 위치 기록이 있으면 **주소**로 변환해 표시합니다. 위치가 없으면 지도에 나타나지 않습니다. |
...
날짜·시각은 MP4 파일명에서도 읽을 수 있지만, **위치는 영상 자체에서 추정하지 않습니다.** SavedClips 또는 SentryClips에 위치 기록이 있을 때 주소를 조회해 지도와 주소 캡션에 표시합니다. 따라서 상시 녹화 영상도 시간 구간 지정·반복 재생·시각 캡션 저장은 가능하지만, 위치 기록이 없으면 주소는 넣지 않습니다.
...
- 전체 녹화 막대 위에 마우스를 올리면 그 위치의 날짜와 시간이 툴팁으로 표시됩니다.
...
이벤트 이름은 `event.json`에서 실제 저장 사유를 읽은 경우에만 표시합니다. 사유 항목이 없거나 파일을 읽지 못한 영상은 **사유 확인 불가**를 큰 제목으로 표시하지 않고 촬영 날짜·시각을 먼저 보여 줍니다. 이름은 파일의 기록을 보여 주며, 영상에서 사고·위험 여부를 자동 판정하는 기능은 아닙니다.

### 타임라인과 지도

영상을 열면 **타임라인**이 먼저 표시되며, 상단 **타임라인 / 목록 / 지도** 버튼으로 전환합니다. 타임라인에서는 날짜를 고르고 원하는 녹화 위치를 누릅니다.
...
아래는 **시각 새김**으로 저장한 영상의 예입니다. 합성 도로 영상과 가상 주소를 사용했습니다.

![날짜·시각과 주소 캡션이 표시된 저장 영상 예](docs/images/overlay-example.webp)

- **지금 화면 캡처:** 선택한 카메라의 장면을 PNG로 저장합니다.
- **원본 그대로:** 원본 파일을 복사합니다. 재인코딩에 따른 화질 손실이 없습니다.
- **시각 새김:** 촬영 날짜·시각과 확인된 주소를 캡션처럼 영상 왼쪽 아래에 표시해 다시 녹화합니다. 주소를 확인할 수 없으면 날짜·시각만 넣습니다. 영상 길이만큼 시간이 걸리므로 탭을 화면에 유지하세요.
...
이 뷰어에서 안전신문고로 직접 전송하지는 않습니다. 지도와 상세 정보에서 **촬영 날짜·시각과 주소**를 확인하고, 필요한 장면을 A/B로 반복 검토한 뒤 **시각 새김** 모드로 해당 구간만 저장해 신고 자료로 첨부할 수 있습니다. 주소는 TeslaCam의 이벤트 위치를 주소 서비스로 변환한 결과이므로, 제출 전에 실제 장소와 영상 내용을 직접 확인하세요. 신고 가능 여부와 파일 형식·용량·접수 기한은 [안전신문고](https://www.safetyreport.go.kr/)의 최신 안내를 따라야 합니다.
